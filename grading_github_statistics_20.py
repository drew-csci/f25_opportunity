...
Usage:
python grading_github_statistics_20.py --repo . --all_refs --repo_full drew-csci/f25_opportunity --start_date 2025-08-25 --end_date 2025-12-12 --org_project_owner drew-csci --org_project_number 3 --user_project_owner alrudniy --user_project_number 7 --name_fix_file fix_github_names.xlsx
...
def load_name_fix_map(path: str) -> Dict[str, str]:
    ...
    try:
        ...
        print(f"  Sample: 'dmitry' in map = {'dmitry' in name_map}, value = {name_map.get('dmitry', 'N/A')}")
...
def count_commits_for_week(
    ...
    # Debug: verify map is received
    if name_fix_map:
        print(f"    DEBUG: name_fix_map has {len(name_fix_map)} entries, 'dmitry' in map = {'dmitry' in name_fix_map}")
    else:
        print(f"    DEBUG: name_fix_map is EMPTY!")
    ...
    for original_name, (display_name, github_username, commits, added, deleted) in raw_data.items():
        ...
        if display_name.lower() in name_fix_map:
            ...
            print(f"WARNING: '{display_name}' (github: '{github_username}') not found in name_fix_map - using as-is")
...
def main() -> None:
    ...
    # Load name fix map
    print(f"DEBUG: args.name_fix_file = '{args.name_fix_file}'")
    name_fix_map = load_name_fix_map(args.name_fix_file) if args.name_fix_file else {}
    print(f"DEBUG: name_fix_map has {len(name_fix_map)} entries after loading")
    if name_fix_map:
